                ...
                for (var j in subcategories) {
                  if (subcategories[j].categories && subcategories[j].categories[categoryId]) {
                    ...
                    for (var k in events) {
                      if (
                        events[k].categories &&
                        events[k].categories[categoryId] &&
                        events[k].subcategories &&
                        events[k].subcategories[subcategoryId]
                      ) {
...
app.post('/deleteImage', bodyParser.urlencoded({ extended: true }), function (request, response) {
  if (request.body.publicid) {
    ...
    cloudinary.v2.uploader.destroy(request.body.publicid, function (error, result) {

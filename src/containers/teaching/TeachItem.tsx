...
const TeachItem: React.FC<TeachItemProps> = ({
  ...
  const detailHtml = useMemo(
    () =>
      isHebrew
        ? teach.detailsHebrew || teach.details || ''
        : teach.details || teach.detailsHebrew || '',
  ...
  const teachImage = typeof teach.image === 'string' ? { src: teach.image } : (teach.image ?? null);
  ...
  return (
    ...
    >
      <CardHeader>
        ...
        {isAuthenticated && isHovering && (
          <div>
            <label htmlFor={`teach-order-${teach.id}`}>{isHebrew ? 'מיקום' : 'Order'}</label>
            <OrderField

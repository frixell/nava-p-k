...
interface ProjectDetailsPageProps {
    ...
    categoryId?: string;
    table?: ProjectCategory[] | null;
    categoryColors?: unknown[];
    lang?: string;
}

const AutosizeInputField = AutosizeInput as unknown as React.ComponentType<any>;
...
const ProjectDetailsPage: React.FC<ProjectDetailsPageProps> = ({
    ...
    const forwardChange = useCallback((name: string, value: string) => {
    ...
    const handleProjectTitle = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
        forwardChange('title', event.target.value);
    }, [forwardChange]);

    const handleTableOptionsChange = useCallback((tableOptions: string) => {
        forwardChange('tableOptions', tableOptions);
    }, [forwardChange]);

    const handleProjectCategoriesChange = useCallback((categoriesValue: string) => {
    ...
    const handleSubcategorySelect = useCallback((event: React.MouseEvent<HTMLDivElement>) => {
        ...
        const subcategoryName = dataTarget.dataset?.name || (event.currentTarget as HTMLElement).dataset?.name;
        if (!subcategoryName) {
            return;
        }
    ...
    return (
        <Container isEnglish={isEnglish}>
            ...
            <PrimaryColumn>
                ...
                <ImageWrapper>
                    {isAuthenticated && (
                        <div className={`backoffice__about__images__buttons${isEnglish ? ' backoffice__about__images__buttons--en' : ' backoffice__about__images__buttons--he'}`}>

    ...
    <CategoriesContainer isMobile={isMobile} isEnglish={isEnglish}>
      {categories.map((category, index) => {
        ...
        const relevantPoints = points.filter((point) => {
          ...
          console.log(
            'point?.categories && point?.categories.length > 0 && point.categories.includes(category.id)',
            point?.categories &&
              point?.categories.length > 0 &&
              point.categories.includes(category.id)
          );
          if (!point.categories) {
            return false;
          }
      ...
      {isAuthenticated && hasUnconnectedProjects ? (
        ...
        >
          ...
          {openCategoryIds.includes('notConnectedProjects') &&
            points

...
export const SidebarContainer = styled('div', filterProps)<{ isMobile: boolean }>(({ isMobile }) => ({
    display: 'flex',
...
export const HeaderSection = styled('div', filterProps)<{ isMobile: boolean; isEnglish: boolean }>(({ isMobile, isEnglish }) => ({
    display: 'flex',
...
export const SidebarImageWrapper = styled('div', filterProps)<{ isMobile: boolean }>(({ isMobile }) => ({
    width: '100%',
    display: 'flex',
...
export const SidebarImage = styled('img', filterProps)<{ isEnglish: boolean }>(({ isEnglish }) => ({
    maxWidth: '100%',
    transform: isEnglish ? 'scaleX(-1)' : 'scaleX(1)'
}));

export const SidebarText = styled('div', filterProps)<{ isEnglish: boolean; isSpaced?: boolean }>(({ isEnglish, isSpaced }) => ({
    width: '100%',
...
export const CategoriesContainer = styled('div', filterProps)<{ isMobile: boolean; isEnglish: boolean }>(({ isMobile, isEnglish }) => ({
    display: 'flex',
    flexDirection: isMobile ? (isEnglish ? 'row' : 'row-reverse') : 'column',
...
export const CategoryWrapper = styled('div', filterProps)<{ isMobile: boolean; isExpanded: boolean; colorHex: string; isSpaced?: boolean }>(({
    isMobile,
    isExpanded,
    colorHex,
    isSpaced
...
export const CategoryHeader = styled('div', filterProps)<{ isEnglish: boolean; isMobile: boolean; colorHex: string }>(({ isEnglish, isMobile, colorHex }) => ({
    display: 'flex',
...
export const ArrowIcon = styled('span', filterProps)<{ isEnglish: boolean; isOpen: boolean }>(({ isEnglish, isOpen }) => {
...
export const CategoryTitle = styled('span', filterProps)<{ isEnglish: boolean }>(({ isEnglish }) => ({
    flex: 1,
    whiteSpace: 'nowrap',
    overflow: 'hidden',
...
export const ProjectItem = styled('div', filterProps)<{ isEnglish: boolean; isSelected: boolean; isEmphasized?: boolean }>(({ isEnglish, isSelected, isEmphasized }) => ({
    width: '100%',
    padding: '5px',
    fontSize: '12px',
    whiteSpace: 'nowrap',

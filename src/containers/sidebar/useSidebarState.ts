...
type SidebarStateParams = Pick<SidebarProps, 'categories' | 'points' | 'language' | 'setOpenCategories' | 'openCategories'>;
...
const useSidebarState = ({
    ...
    useEffect(() => {
        ...
        setOpenCategoryIds((prev: string[]) => {
            ...
            const hasDifference =
                incoming.length !== prev.length || incoming.some((id: string, index: number) => id !== prev[index]);

...
const normalizeCategory = (id: string, value: Partial<Category> | null | undefined): Category => {
  ...
  const isVisible = typeof value?.isVisible === 'boolean' ? value.isVisible : Boolean(value?.isVisible);
...
const categoriesSlice = createSlice({
  ...
  initialState,
  reducers: {
    setCategories: (_state: CategoriesState, action: PayloadAction<CategoriesState>) => action.payload,
    addCategory: (state: CategoriesState, action: PayloadAction<Category>) => {
      state.push(action.payload);
    },
    updateCategories: (_state: CategoriesState, action: PayloadAction<CategoriesState>) => action.payload,
...
export const startGetCategories = (): AppThunk<Promise<void>> => async (dispatch: AppDispatch) => {
  ...
  snapshot.forEach((childSnapshot: DataSnapshot) => {
    ...
    if (categoryData) {
      categories.push(
        normalizeCategory(childSnapshot.key ?? '', categoryData)
      );
    }
    return false;
  });

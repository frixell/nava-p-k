...
const normalizeTableOption = (id: string, value: Partial<TableOption> | null | undefined): TableOption => ({
    id,
...
const tableTemplateSlice = createSlice({
    ...
    reducers: {
        setTableTemplate: (
            _state: TableTemplateState,
            action: PayloadAction<TableTemplateState>
        ) => action.payload
    }
});
...
export const startGetTableTemplate = (): AppThunk<Promise<void>> => async (dispatch: AppDispatch) => {
    ...
    snapshot.forEach((childSnapshot: firebase.database.DataSnapshot) => {
        ...
        tableTemplate.push(
            normalizeTableCategory(childSnapshot.key ?? '', categoryData)
        );
